...
def planarAR(REF_IMAGE_PATH, VIDEO_PATH):
    ...
    while (video.isOpened()):
        ...
        if ret:  ## check whethere the frame is legal, i.e., there still exists a frame
            ...
            corners, _, _ = cv2.aruco.detectMarkers(frame, arucoDict, parameters=arucoParameters)
            ...
            ymin, ymax, xmin, xmax = np.min(corners[:,1]), np.max(corners[:,1]), np.min(corners[:,0]), np.max(corners[:,0])
            warping(ref_image, frame, H, ymin, ymax, xmin, xmax, 'b')
